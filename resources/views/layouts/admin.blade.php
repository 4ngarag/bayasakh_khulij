...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    <head>
        ...
        <script src="https://cdn.tiny.cloud/1/ge6diian0s7kgofpak9opaxkj49vxpsu1wzl0oz04a6soja5/tinymce/6/tinymce.min.js" referrerpolicy="origin"></script>
    </head>
        ...
        <div @click.away="open = false" class="flex flex-col flex-shrink-0 w-full text-gray-700 bg-slate-100 md:w-64 dark-mode:text-gray-200 dark-mode:bg-gray-800" x-data="{ open: false }">
            <div class="flex flex-row items-center justify-between flex-shrink-0 px-8 py-4">
                ...
                <nav :class="{'block': open, 'hidden': !open}" class="flex-grow px-4 pb-4 md:block md:pb-0 md:overflow-y-auto">
                    <x-admin-nav-link :href="route('admin.posts.index')" :active="request()->routeIs('admin.posts.index')">
                        {{ __('Posts') }}
                    </x-admin-nav-link>
                    <x-admin-nav-link :href="route('admin.companies.index')" :active="request()->routeIs('admin.companies.index')">
                        {{ __('Company') }}
                    </x-admin-nav-link>
    ...
    {{-- <script src="{{ asset('src/js/tinymce/tinymce.min.js') }}"></script> --}}
    <script>
        tinymce.init({
          selector: '#texteditor',
